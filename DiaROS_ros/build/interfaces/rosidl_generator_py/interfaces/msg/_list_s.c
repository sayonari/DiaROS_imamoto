...
bool interfaces__msg__list__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("interfaces.msg._list.List", full_classname_dest, 25) == 0);
  }
  interfaces__msg__List * ros_message = _ros_message;
  {  // n
  ...
  {  // bot
    PyObject * field = PyObject_GetAttrString(_pymsg, "bot");
    if (!field) {
      return false;
    }
    assert(PyUnicode_Check(field));
    ...
    rosidl_runtime_c__String__assign(&ros_message->bot, PyBytes_AS_STRING(encoded_field));
    Py_DECREF(encoded_field);
    Py_DECREF(field);
  }
  {  // frequency
...
{
  /* NOTE(esteve): Call constructor of List */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("interfaces.msg._list");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "List");
  ...
  {  // bot
    ...
    {
      int rc = PyObject_SetAttrString(_pymessage, "bot", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
